...
for n_estim in num_estimators:
    ...
    for days in days_hist:
        ...
        setting_name = dataset_name + "_" + str(days) + "_IF_num_estimators_" + str(n_estim)
        ...
        for pred_day in dates:
            ...
            isolation_forest_predictions = isolation_forest.predict(pred_data_without_date)
            isolation_forest_predictions = np.where(isolation_forest_predictions == 1, 0, isolation_forest_predictions)
            isolation_forest_predictions = np.where(isolation_forest_predictions == -1, 1, isolation_forest_predictions)
            ...
            np.savetxt('logs/' + setting_name + "/if.preds_" + str(test_day) + ".log", pred_real_if, delimiter=',', fmt='%s')
            test_day = test_day + 1
